...
func (node *Node) Html(self bool) string {
	var buf bytes.Buffer
	if self {
		html.Render(&buf, node.proto)
	} else {
		for n := node.proto.FirstChild; n != nil; n = n.NextSibling {
			html.Render(&buf, n)
		}
	}
	return buf.String()
...
func ParseHTML(r io.Reader) (*Node, error) {
	...
	n := ns[0]
	// The root node.
	nodes = append(nodes, Node{kind: startNode, proto: n})
	...
	for n != nil {
		...
		for n != nil {
			if n.Type == html.ElementNode {
				nodes = append(nodes, Node{kind: endNode, proto: n})
			}
